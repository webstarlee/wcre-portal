...
const EditSale: React.FC<EditSaleProps> = ({
  ...
  const [newSale, setNewSale] = React.useState<SaleProps>({
    ...
    sale_state: stateName(sale.sale_state),
    sale_cover: sale.sale_cover,
    ...
    sale_end_date_days: dayjs(sale.sale_end_date),
    sale_property_type: sale.sale_property_type,
    sale_type: sale.sale_type,
    sale_price: removeSymbol(sale.sale_price),
    ...
    broker_users: sale.broker_users,
  ...
  const [saleCover, setSaleCover] = React.useState<string | undefined>(
    undefined
  );
  ...
  const getBucketImage = async (key: string) => {
    try {
      ...
      if (url) {
        setSaleCover(formatUrl(url));
      } else {
        setSaleCover(ListingImgUrl);
      }
    } catch (error) {
      setSaleCover(ListingImgUrl);
    }
  ...
  const handleUpdateSale = async () => {
    ...
    if (newSale.cover || newSale.sale_agreement) {
      ...
      axios
        .post(convertApiUrl("sale/update"), newSale, {
          headers: {
            Authorization: `Bearer ${authToken}`,
          },
        })
          ...
          {step === 1 && (
            <Box sx={{ width: "100%" }}>
              <Box sx={{ width: "100%", marginBottom: "20px" }}>
                ...
                >
                  Update Sale - Property Information
                </Typography>
              </Box>
              {!newSale.cover && (
                <ImageUploadButton
                  sx={{
                    padding: sale.sale_cover !== "" ? "0px" : "6px 8px",
                    border:
                      sale.sale_cover !== "" ? "0px" : "dashed 2px #B9BABB",
                  }}
                  onClick={triggerCoverInput}
                >
                  {sale.sale_cover !== "" ? (
                    <ImageUploadCoverImg src={saleCover} alt="upload" />
                  ) : (
                    <>
          ...
          {step === 4 && (
            <Box sx={{ width: "100%" }}>
              <Box sx={{ width: "100%", marginBottom: "20px" }}>
                ...
                >
                  Update Sale - General Information
                </Typography>
              </Box>
              <Box
                sx={{
